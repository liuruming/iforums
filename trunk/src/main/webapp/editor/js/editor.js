...
function newEditor(mode, initialtext) {
	...
	if (wysiwyg) {
		...
		writeEditorContents(isUndefined(initialtext) ? textobj.value
				: initialtext);
	} else {
...
function initEditor() {
		...
		if (buttons[i].id.indexOf(editorid + '_') != -1) {
			...
			if (buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'fullswitcher') {
				...
				buttons[i].onmouseover = function(e) {
					setEditorTip(editorisfull ? '恢复编辑器大小' : '全屏方式编辑');
				};
				...
				_attachEvent(buttons[i], 'mouseover', function(e) {
					setEditorTip(BROWSER.ie ? window.event.srcElement.title
							: e.target.title);
				});
				if (buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'url') {
					...
					if (!buttons[i].getAttribute('init')) {
						buttons[i].onclick = function(e) {
							discuzcode(this.id.substr(this.id.indexOf('_') + 1));
							doane();
						};
					}
				}
	...
	textobj.onkeydown = function(e) {
		ctlent(e ? e : event)
	};
	if (editorcontroltop === false
			&& (BROWSER.ie && BROWSER.ie > 6 || !BROWSER.ie)) {
		seteditorcontrolpos();
		var obj = wysiwyg ? editwin.document.body.parentNode : $(editorid
				+ '_textarea');
		...
		$(editorid + '_controls').parentNode.insertBefore(ctrlmObj, $(editorid
				+ '_controls'));
	...
	if ($(editorid + '_svdsecond') && savedatat === null) {
		...
		autosave = !getcookie('editorautosave_' + editorid)
				|| getcookie('editorautosave_' + editorid) == 1 ? 1 : 0;
		savedataTime();
...
function savedataTime() {
	...
	$(editorid + '_svdsecond').innerHTML = '<a title="点击关闭自动保存" href="javascript:;" onclick="setAutosave()">'
			+ savedatac + ' 秒后保存</a> ';
	savedatac -= 10;
}
...
function editorcontrolpos() {
	...
	if (scrollTop > editorcontroltop && editorcurrentheight > editorminheight) {
		...
		$(editorid + '_controls').style.position = $(editorid + '_controls').style.top = $(editorid
				+ '_controls').style.width = '';
...
function editorsize(op, v) {
	...
	editorcurrentheight = editorheight > editorminheight ? editorheight
			: editorminheight;
	if ($(editorid + '_iframe')) {
		$(editorid + '_iframe').style.height = $(editorid + '_iframe').contentWindow.document.body.style.height = editorcurrentheight
				+ 'px';
	}
	if (framemObj) {
...
function editorresize(e, op) {
	...
	if (op == 1) {
		if (wysiwyg) {
			...
			framemObj.style.height = $(editorid + '_iframe').clientHeight
					+ 'px';
...
function editorfull(op) {
	var op = !op ? 0 : op, control = $(editorid + '_controls'), area = $(editorid
			+ '_textarea').parentNode, bbar = $(editorid + '_bbar'), iswysiwyg = wysiwyg;
	if (op) {
		var editorheight = document.documentElement.clientHeight
				- control.offsetHeight
				- bbar.offsetHeight
				- parseInt(getCurrentStyle(area, 'paddingTop', 'padding-top'))
				- parseInt(getCurrentStyle(area, 'paddingBottom',
						'padding-bottom'));
		...
		bbar.style.top = (document.documentElement.clientHeight - bbar.offsetHeight)
				+ 'px';
		return;
	}
	if (iswysiwyg) {
	...
	if (!editorisfull) {
		...
		area.style.backgroundColor = $(editorid + '_textarea') ? getCurrentStyle(
				$(editorid + '_textarea'), 'backgroundColor',
				'background-color')
				: '#fff';
		...
		control.style.position = control.style.top = control.style.left = control.style.width = control.style.minWidth = control.style.zIndex = area.style.position = area.style.top = area.style.left = area.style.width = area.style.height = area.style.zIndex = bbar.style.position = bbar.style.top = bbar.style.left = bbar.style.width = bbar.style.zIndex = '';
...
function pasteWord(str) {
	...
	if (mstest.test(str)) {
		...
		str = str.replace(/<(\w[^>]*) style="([^"]*)"([^>]*)/gi, function($1,
				$2, $3, $4) {
			var style = '';
			...
			match = re.exec($3);
			if (match != null) {
				style += 'text-indent:' + parseInt(parseInt(match[2]) / 10)
						+ 'em;';
			}
...
function ctlent(event) {
	if (postSubmited == false && (event.ctrlKey && event.keyCode == 13)
			|| (event.altKey && event.keyCode == 83) && editorsubmit) {
		if (in_array(editorsubmit.name, [ 'topicsubmit', 'replysubmit',
				'editsubmit' ])
				&& !validate(editorform)) {
			doane(event);
			return;
...
function checkFocus() {
	var obj = wysiwyg ? (!BROWSER.chrome ? editwin.document.body : editwin)
			: textobj;
	if (!obj.hasfocus) {
...
function checklength(theform) {
	var message = wysiwyg ? html2bbcode(getEditorContents())
			: (!theform.parseurloff.checked ? parseurl(theform.message.value)
					: theform.message.value);
	showDialog('当前长度: '
			+ mb_strlen(message)
			+ ' 字节，'
			+ (postmaxchars != 0 ? '系统限制: ' + postminchars + ' 到 '
					+ postmaxchars + ' 字节。' : ''), 'notice', '字数检查');
}
...
function writeEditorContents(text) {
	if (wysiwyg) {
		...
		if (initialized
				&& !(BROWSER.firefox && BROWSER.firefox >= '3' || BROWSER.opera)) {
			editdoc.body.innerHTML = text;
		} else {
			text = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">'
					+ '<html><head id="editorheader"><meta http-equiv="Content-Type" content="text/html; charset='
					+ charset
					+ '" />'
					+ (BROWSER.ie && BROWSER.ie > 7 ? '<meta http-equiv="X-UA-Compatible" content="IE=7" />'
							: '')
					+ '<link rel="stylesheet" type="text/css" href="data/cache/style_'
					+ STYLEID
					+ '_wysiwyg.css?'
					+ VERHASH
					+ '" />'
...
function wrapTags(tagname, useoption, selection) {
	...
	insertText(text, strlen(opentag), strlen(closetag), in_array(tagname, [
			'code', 'quote', 'free', 'hide' ]) ? true : false);
}
function applyFormat(cmd, dialog, argument) {
	if (wysiwyg) {
		editdoc.execCommand(cmd, (isUndefined(dialog) ? false : dialog),
				(isUndefined(argument) ? true : argument));
		return;
	}
	switch (cmd) {
	case 'paste':
...
function insertAttachimgTag(aid) {
	if (wysiwyg) {
		insertText('<img src="' + $('image_' + aid).src
				+ '" border="0" aid="attachimg_' + aid + '" alt="" />', false);
	} else {
...
function discuzcode(cmd, arg) {
	...
	checkFocus();
	if (in_array(cmd, [ 'sml', 'url', 'quote', 'code', 'free', 'hide', 'aud',
			'vid', 'fls', 'attach', 'image', 'pasteword' ])
			|| cmd == 'tbl'
			|| in_array(cmd,
		...
		if ((str = getSnapshot()) !== false) {
			...
			&& in_array(cmd, [ 'insertorderedlist', 'insertunorderedlist' ])) {
		...
		if (wysiwyg) {
			if (txt = getSel()) {
				...
				insertText(
						'<br style="clear: both"><table class="float" style="float: '
								+ arg + '; margin-' + argm
								+ ': 5px;"><tbody><tr><td>' + txt
		...
		showDialog(
				'<div id="remotedowninfo"><p class="mbn">正在下载远程附件，请稍等……</p><p><img src="'
						+ STATICURL
						+ 'image/common/uploading.gif" alt="" /></p></div>',
				'notice', '', null, 1);
		var message = wysiwyg ? html2bbcode(getEditorContents())
				: (!editorform.parseurloff.checked ? parseurl(editorform.message.value)
						: editorform.message.value);
		var oldValidate = editorform.onsubmit;
		var oldAction = editorform.action;
		editorform.onsubmit = '';
		editorform.action = 'forum.php?mod=ajax&action=downremoteimg&fid='
				+ fid + '&wysiwyg=' + (wysiwyg ? 1 : 0);
		...
		try {
			var ret = applyFormat(formatcmd, false, (isUndefined(arg) ? true
					: arg));
		} catch (e) {
			var ret = false;
		}
	...
	if (in_array(cmd, [ 'bold', 'italic', 'underline', 'strikethrough',
			'inserthorizontalrule', 'fontname', 'fontsize', 'forecolor',
			'backcolor', 'justifyleft', 'justifycenter', 'justifyright',
			'insertorderedlist', 'insertunorderedlist', 'floatleft',
			'floatright', 'removeformat', 'unlink', 'undo', 'redo' ])) {
		hideMenu();
	}
	doane();
	return ret;
...
function setContext(cmd) {
	var cmd = !cmd ? '' : cmd;
	var contextcontrols = new Array('bold', 'italic', 'underline',
...
function showEditorMenu(tag, params) {
	...
	var menutype = 'menu';
	if (BROWSER.ie) {
		sel = wysiwyg ? editdoc.selection.createRange() : document.selection
				.createRange();
		pos = getCaret();
	}
	...
	if (menu) {
		...
		if (menupos == '00') {
			...
			showMenu({
				...
				'pos' : menupos,
				'timeout' : 250,
				'duration' : in_array(tag, [ 'fontname', 'fontsize', 'sml' ]) ? 2
						: 3,
				'drag' : 1
			});
		}
		...
		switch (tag) {
		case 'url':
			str = '请输入链接地址:<br /><input type="text" id="'
					+ ctrlid
					+ '_param_1" style="width: 98%" value="" class="px" />'
					+ (selection ? ''
							: '<br />请输入链接文字:<br /><input type="text" id="'
									+ ctrlid
									+ '_param_2" style="width: 98%" value="" class="px" />');
			break;
		case 'forecolor':
		...
		case 'free':
			if (selection) {
				return insertText((opentag + selection + closetag),
						strlen(opentag), strlen(closetag), true, sel);
			}
			var lang = {
				'quote' : '请输入要插入的引用',
				'code' : '请输入要插入的代码',
			...
			str += lang[tag]
					+ ':<br /><textarea id="'
					+ ctrlid
					+ '_param_1" style="width: 98%" cols="50" rows="5" class="txtarea"></textarea>'
					+ (tag == 'hide' ? '<br /><label><input type="radio" name="'
							+ ctrlid
							+ '_radio" id="'
							+ ctrlid
							+ '_radio_1" class="pc" checked="checked" />只有当浏览者回复本帖时才显示</label><br /><label><input type="radio" name="'
							+ ctrlid
							+ '_radio" id="'
							+ ctrlid
							+ '_radio_2" class="pc" />只有当浏览者积分高于</label> <input type="text" size="3" id="'
							+ ctrlid + '_param_2" class="px pxs" /> 时才显示'
							: '');
			break;
		case 'tbl':
			str = '<p class="pbn">表格行数: <input type="text" id="'
					+ ctrlid
					+ '_param_1" size="2" value="2" class="px" /> &nbsp; 表格列数: <input type="text" id="'
					+ ctrlid
					+ '_param_2" size="2" value="2" class="px" /></p><p class="pbn">表格宽度: <input type="text" id="'
					+ ctrlid
					+ '_param_3" size="2" value="" class="px" /> &nbsp; 背景颜色: <input type="text" id="'
					+ ctrlid
					+ '_param_4" size="2" class="px" onclick="showColorBox(this.id, 2)" /></p><p class="xg2 pbn" style="cursor:pointer" onclick="showDialog($(\'tbltips_msg\').innerHTML, \'notice\', \'小提示\', null, 0)"><img id="tbltips" title="小提示" class="vm" src="'
					+ IMGDIR + '/info_small.gif"> 快速书写表格提示</p>';
			...
			break;
		case 'aud':
			str = '<p class="pbn">请输入音乐文件地址:</p><p class="pbn"><input type="text" id="'
					+ ctrlid
					+ '_param_1" class="px" value="" style="width: 220px;" /></p><p class="xg2 pbn">支持 wma mp3 ra rm 等音乐格式<br />示例: http://server/audio.wma</p>';
			break;
		case 'vid':
			str = '<p class="pbn">请输入视频地址:</p><p class="pbn"><input type="text" value="" id="'
					+ ctrlid
					+ '_param_1" style="width: 220px;" class="px" /></p><p class="pbn">宽: <input id="'
					+ ctrlid
					+ '_param_2" size="5" value="500" class="px" /> &nbsp; 高: <input id="'
					+ ctrlid
					+ '_param_3" size="5" value="375" class="px" /></p><p class="xg2 pbn">支持优酷、土豆、56、酷6等视频站的视频网址<br />支持 wmv avi rmvb mov swf flv 等视频格式<br />示例: http://server/movie.wmv</p>';
			break;
		case 'fls':
			str = '<p class="pbn">请输入 Flash 文件地址:</p><p class="pbn"><input type="text" id="'
					+ ctrlid
					+ '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn">宽: <input id="'
					+ ctrlid
					+ '_param_2" size="5" value="" class="px" /> &nbsp; 高: <input id="'
					+ ctrlid
					+ '_param_3" size="5" value="" class="px" /></p><p class="xg2 pbn">支持 swf flv 等 Flash 网址<br />示例: http://server/flash.swf</p>';
			break;
		case 'pasteword':
			stitle = '从 Word 粘贴内容';
			str = '<p class="px" style="height:300px"><iframe id="'
					+ ctrlid
					+ '_param_1" frameborder="0" style="width:100%;height:100%" onload="this.contentWindow.document.body.style.width=\'550px\';this.contentWindow.document.body.contentEditable=true;this.contentWindow.document.body.focus();this.onload=null"></iframe></p><p class="xg2 pbn">请通过快捷键(Ctrl+V)把 Word 文件中的内容粘贴到上方</p>';
			menuwidth = 600;
			menupos = '00';
			menutype = 'win';
			break;
		default:
			var haveSel = selection == null
					|| selection == false
					|| in_array(trim(selection), [ '', 'null', 'undefined',
							'false' ]) ? 0 : 1;
			if (params == 1 && haveSel) {
				return insertText((opentag + selection + closetag),
						strlen(opentag), strlen(closetag), true, sel);
			...
			for ( var i = 1; i <= params; i++) {
				if (i != params || !haveSel) {
					str += (promptlang[i - 1] ? promptlang[i - 1] : '请输入第 ' + i
							+ ' 个参数:')
							+ '<br /><input type="text" id="'
							+ ctrlid
							+ '_param_'
							+ i
							+ '" style="width: 98%" value="" class="px" />'
							+ (i < params ? '<br />' : '');
				}
			}
			break;
		...
		if (menupos == '00') {
			...
			s = '<table width="100%" cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"></td><td class="t_r"></td></tr><tr><td class="m_l">&nbsp;&nbsp;</td><td class="m_c">'
					+ '<h3 class="flb"><em>'
					+ stitle
					+ '</em><span><a onclick="hideMenu(\'\', \'win\');return false;" class="flbc" href="javascript:;">关闭</a></span></h3><div class="c">'
					+ str
					+ '</div>'
					+ '<p class="o pns"><button type="submit" id="'
					+ ctrlid
					+ '_submit" class="pn pnc"><strong>提交</strong></button></p>'
					+ '</td><td class="m_r"></td></tr><tr><td class="b_l"></td><td class="b_c"></td><td class="b_r"></td></tr></table>';
		} else {
			s = '<div class="p_opt cl"><span class="y" style="margin:-10px -10px 0 0"><a onclick="hideMenu();return false;" class="flbc" href="javascript:;">关闭</a></span><div>'
					+ str
					+ '</div><div class="pns mtn"><button type="submit" id="'
					+ ctrlid
					+ '_submit" class="pn pnc"><strong>提交</strong></button></div></div>';
		}
		menu.innerHTML = s;
	...
	for ( var i = 0; i < objs.length; i++) {
		_attachEvent(
				...
				function(e) {
					...
					if ((obj.type == 'text' && e.keyCode == 13)
							|| (obj.type == 'textarea' && e.ctrlKey && e.keyCode == 13)) {
		...
		$(ctrlid + '_submit').onclick = function() {
				...
				if (href != '') {
					var v = selection ? selection
							: ($(ctrlid + '_param_2').value ? $(ctrlid
									+ '_param_2').value : href);
					str = wysiwyg ? ('<a href="' + href + '">' + v + '</a>')
							: '[url=' + href + ']' + v + '[/url]';
					if (wysiwyg)
						insertText(str, str.length - v.length, 0,
								(selection ? true : false), sel);
					else
						insertText(str, str.length - v.length - 6, 6,
								(selection ? true : false), sel);
				}
				break;
			case 'code':
			...
			case 'free':
				if (tag == 'hide' && $(ctrlid + '_radio_2').checked) {
					...
					opentag = mincredits > 0 ? '[hide=' + mincredits + ']'
							: '[hide]';
				}
				str = $(ctrlid + '_param_1') && $(ctrlid + '_param_1').value ? $(ctrlid
						+ '_param_1').value
						: (selection ? selection : '');
				if (wysiwyg) {
			...
			case 'tbl':
				...
				rows = /^[-\+]?\d+$/.test(rows) && rows > 0 && rows <= 30 ? rows
						: 2;
				columns = /^[-\+]?\d+$/.test(columns) && columns > 0
						&& columns <= 30 ? columns : 2;
				width = width.substr(width.length - 1, width.length) == '%' ? (width
						.substr(0, width.length - 1) <= 98 ? width : '98%')
						: (width <= 560 ? width : '98%');
				bgcolor = /[\(\)%,#\w]+/.test(bgcolor) ? bgcolor : '';
				if (wysiwyg) {
					str = '<table cellspacing="0" cellpadding="0" width="'
							+ (width ? width : '50%') + '" class="t_table"'
				...
				break;
			case 'aud':
				insertText('[audio]' + $(ctrlid + '_param_1').value
						+ '[/audio]', 7, 8, false, sel);
				break;
			case 'fls':
				if ($(ctrlid + '_param_2').value
						&& $(ctrlid + '_param_3').value) {
					insertText('[flash='
							+ parseInt($(ctrlid + '_param_2').value) + ','
							+ parseInt($(ctrlid + '_param_3').value) + ']'
							+ $(ctrlid + '_param_1').value + '[/flash]', 7, 8,
							false, sel);
				} else {
					insertText('[flash]' + $(ctrlid + '_param_1').value
							+ '[/flash]', 7, 8, false, sel);
				}
				break;
			case 'vid':
				...
				var auto = '';
				var ext = mediaUrl.lastIndexOf('.') == -1 ? '' : mediaUrl
						.substr(mediaUrl.lastIndexOf('.') + 1,
								mb_strlen(mediaUrl)).toLowerCase();
				ext = in_array(ext, [ 'mp3', 'wma', 'ra', 'rm', 'ram', 'mid',
						'asx', 'wmv', 'avi', 'mpg', 'mpeg', 'rmvb', 'asf',
						'mov', 'flv', 'swf' ]) ? ext : 'x';
				if (ext == 'x') {
				...
				var str = '[media=' + ext + ',' + $(ctrlid + '_param_2').value
						+ ',' + $(ctrlid + '_param_3').value + ']' + mediaUrl
						+ '[/media]';
				insertText(str, str.length, 0, false, sel);
				break;
			case 'image':
				...
				if (wysiwyg) {
					...
					insertText('[img' + style + ']' + src + '[/img]', 0, 0,
							false, sel);
				}
			...
			default:
				...
				if ((params == 1 && first)
						|| (params == 2 && first && (haveSel || second))
						|| (params == 3 && first && second && (haveSel || third))) {
					if (params == 1) {
						str = first;
					} else if (params == 2) {
					...
					insertText((opentag + str + closetag), strlen(opentag),
							strlen(closetag), true, sel);
				}
				break;
			}
			hideMenu();
...
function autoTypeset() {
	...
	var selection = sel ? (wysiwyg ? sel.htmlText.replace(/<\/?p>/ig, '<br />')
			: sel.text) : getSel();
	selection = wysiwyg ? selection.replace(/<br[^\>]*>/ig, "\n") : selection
			.replace(/\r?\n/g, "\n");
	selection = trim(selection);
	selection = wysiwyg ? selection.replace(/\n\n+/g,
			'</p><p style="line-height: 30px; text-indent: 2em;">') : selection
			.replace(/\n/g, '[/p][p=30, 2, left]');
	opentag = wysiwyg ? '<p style="line-height: 30px; text-indent: 2em;">'
			: '[p=30, 2, left]';
	...
	if (wysiwyg) {
		try {
			...
			if (!isUndefined(editdoc.selection)
					&& editdoc.selection.type != 'Text'
					&& editdoc.selection.type != 'None') {
				movestart = false;
		...
		if (!isUndefined(editdoc.selectionStart)) {
			...
			editdoc.value = editdoc.value.substr(0, editdoc.selectionStart)
					+ text + editdoc.value.substr(editdoc.selectionEnd);
			if (!isUndefined(movestart)) {
...
function stripSimple(tag, str, iterations) {
	...
	while ((startindex = stripos(str, opentag)) !== false && iterations != 0) {
		...
		if ((stopindex = stripos(str, closetag)) !== false) {
			var text = str.substr(startindex + opentag.length, stopindex
					- startindex - opentag.length);
			str = str.substr(0, startindex) + text
					+ str.substr(stopindex + closetag.length);
		} else {
			break;
		}
	}
...
function stripComplex(tag, str, iterations) {
	...
	while ((startindex = stripos(str, opentag)) !== false && iterations != 0) {
		...
		if ((stopindex = stripos(str, closetag)) !== false) {
			...
			if (openend !== false && openend > startindex
					&& openend < stopindex) {
