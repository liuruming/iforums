...
if (BROWSER.ie) {
	BROWSER.iemode = parseInt(typeof document.documentMode != 'undefined' ? document.documentMode
			: BROWSER.ie);
...
function preg_replace(search, replace, str, regswitch) {
	...
	for ( var i = 0; i < len; i++) {
		...
		str = str.replace(re, typeof replace == 'string' ? replace
				: (replace[i] ? replace[i] : replace[0]));
	}
	return str;
}
		...
		if (!safescripts[id] || !index) {
			...
			safescripts[id].push({
				...
				'si' : setInterval(function() {
					safescript(id, call, seconds, times, timeoutcall, endcall,
							safescripts[id].length);
				}, seconds)
			});
		} else {
			index = (index || 1) - 1;
...
function ajaxmenu(ctrlObj, timeout, cache, duration, pos, recall, idclass,
	...
	if (menu) {
		...
		menu.innerHTML = '<div class="' + contentclass + '" id="' + menuid
				+ '_content"></div>';
...
function loadcss(cssname) {
	if (!CSSLOADED[cssname]) {
		if (!$('css_' + cssname)) {
			...
			css.rel = 'stylesheet';
			css.href = 'data/cache/style_' + STYLEID + '_' + cssname + '.css?'
					+ VERHASH;
			var headNode = document.getElementsByTagName("head")[0];
			headNode.appendChild(css);
		} else {
			$('css_' + cssname).href = 'data/cache/style_' + STYLEID + '_'
					+ cssname + '.css?' + VERHASH;
		}
		CSSLOADED[cssname] = 1;
	}
...
function showMenu(v) {
	...
	if (!menuObj.getAttribute('initialized')) {
		...
		if (cover) {
			...
			coverObj.style.height = Math.max(
...
function hideMenu(attr, mtype) {
	...
	if (attr == '') {
		...
		if ((ctrlObj = $(menuObj.getAttribute('ctrlid')))
		...
		var hide = function() {
			if (menuObj.cache) {
				...
				if (menuObj.cover)
					$(attr + '_cover').parentNode
							.removeChild($(attr + '_cover'));
			}
			var tmp = [];
...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt,
		canceltxt, closetime, locationtime) {
	...
	if (mode == 'info') {
		...
		s += '<div class="c altw"><div class="'
				+ (mode == 'alert' ? 'alert_error'
						: (mode == 'right' ? 'alert_right' : 'alert_info'))
				+ '"><p>' + msg + '</p></div></div>';
		s += '<p class="o pns">'
...
function showWindow(k, url, mode, cache, menuv) {
	...
	if (!menuObj) {
		...
		menuObj.style.display = 'none';
		$('append_parent').appendChild(menuObj);
		evt = ' style="cursor:move" onmousedown="dragMenu($(\'' + menuid
				+ '\'), event, 1)" ondblclick="hideWindow(\'' + k + '\')"';
		if (!BROWSER.ie) {
			hidedom = '<style type="text/css">object{visibility:hidden;}</style>';
		}
		menuObj.innerHTML = hidedom
				+ '<table cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"'
				+ evt + '></td><td class="t_r"></td></tr><tr><td class="m_l"'
				+ evt + ')">&nbsp;&nbsp;</td><td class="m_c" id="fwin_content_'
				+ k + '">' + '</td><td class="m_r"' + evt
				+ '"></td></tr><tr><td class="b_l"></td><td class="b_c"' + evt
...
function simulateSelect(selectId, widthvalue) {
	...
	var selectwidth = (selectObj.getAttribute('width', i) ? selectObj
			...
			+ 'px';
	var tabindex = selectObj.getAttribute('tabindex', i) ? selectObj
			.getAttribute('tabindex', i) : 1;
	...
	selectObj.outerHTML += '<a href="javascript:;" id="' + selectId
			+ '_ctrl" style="width:' + selectwidth + '" tabindex="' + tabindex
			+ '">' + defaultopt + '</a>';
	...
	$(selectId + '_ctrl').onclick = function(e) {
		...
		showMenu({
			'ctrlid' : (selectId == 'loginfield' ? 'account' : selectId
					+ '_ctrl'),
...
function parseurl(str, mode, parsecode) {
	if (isUndefined(parsecode))
		parsecode = true;
	if (parsecode)
		str = str.replace(/\s*\[code\]([\s\S]+?)\[\/code\]\s*/ig, function($1,
				$2) {
			return codetag($2);
		});
	str = str
			.replace(
					...
					mode == 'html' ? '$1<a href="$2" target="_blank">$2</a>'
							: '$1[url]$2[/url]');
	str = str
			.replace(
					...
					mode == 'html' ? '$1<a href="$2" target="_blank">$2</a>'
							: '$1[url]$2[/url]');
	str = str.replace(/([^\w->=\]:"'\.\/]|^)(([\-\.\w]+@[\.\-\w]+(\.\w+)+))/ig,
			mode == 'html' ? '$1<a href="mailto:$2">$2</a>'
					: '$1[email]$2[/email]');
	if (parsecode) {
		for ( var i = 0; i <= DISCUZCODE['num']; i++) {
			str = str.replace("[\tDISCUZ_CODE_" + i + "\t]",
					DISCUZCODE['html'][i]);
		}
	}
	return str;
...
function initSearchmenu(searchform, cloudSearchUrl) {
	...
	if (typeof cloudSearchUrl == "undefined" || cloudSearchUrl == null
			|| cloudSearchUrl == '') {
...
function noticeTitleFlash() {
	if (NOTICETITLE.flashNumber < 5 || NOTICETITLE.flashNumber > 4
			&& !NOTICETITLE['State']) {
		document.title = (NOTICETITLE['State'] ? '【　　　】' : '【新提醒】')
				+ NOTICETITLE['oldTitle'];
		NOTICETITLE['State'] = !NOTICETITLE['State'];
	}
	NOTICETITLE.flashNumber = NOTICETITLE.flashNumber < NOTICETITLE.sleep ? ++NOTICETITLE.flashNumber
			: 0;
}
...
function showTopLink() {
	if ($('ft')) {
		...
		if (BROWSER.ie && BROWSER.ie < 7) {
			$('scrolltop').style.top = viewPortHeight - scrollHeight - 150
					+ 'px';
		}
		if (scrollHeight < -100) {
